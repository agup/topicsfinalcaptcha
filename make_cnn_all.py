...
def batch_generator(images, labels, batch_size):
    ...
    while True:
        ...
        X.append(im)
        y_lab = labels[inds[i]]
        y_lab_vec = np.zeros((36*4,))
        y_lab_vec[alph_dict[y_lab[0]]] = 1
...
def loss( digits_logits,  digits_labels):
    ...
    digit1_cross_entropy = tf.reduce_mean(tf.losses.softmax_cross_entropy(onehot_labels=lab1, logits=digits_logits[0]))
...
labelspl = tf.placeholder(tf.int32, [None, 36*4])
...
lab1, lab2, lab3, lab4 = tf.split(labelspl, num_or_size_splits = 4, axis = 1)
...
acc1 = tf.count_nonzero(tf.subtract(a1, tf.argmax(dl[0], 1)))
...
for i in range(0, 10000000):
    ...
    y = np.reshape(y, ( b_size, 36*4 ))
    feed_dict = {imagepl : X, labelspl :y }
    a, lo, t, dell = sess.run([acc1,  los, train_op, dl], feed_dict = feed_dict)
    if i %1000 == 0:
        np.save('acccu_all_db.npy', accs)
        np.save('loss_all_db.npy',run_loss)
        saver.save(sess, '/home/ubuntu/cnn_all_db_wide')
